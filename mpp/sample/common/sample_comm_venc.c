...
******************************************************************************/
HI_S32 SAMPLE_COMM_YJSNPIVENC_SaveStreamToFile(FILE* pFd, VENC_STREAM_S* pstStream)
{
    HI_S32 i;
            ...
            if (SAMPLE_RC_CBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH265Cbr.u32BitRate = 1024 * 5 + 3072*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH265Vbr.u32MaxBitRate = 1024 * 5 + 3072*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
            ...
            else if(SAMPLE_RC_AVBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH265AVbr.u32MaxBitRate = 1024 * 5 + 3072*u32FrameRate/30;	// ~8M bitrate
                        break;
                    case PIC_2592x1944:
            ...
            else if(SAMPLE_RC_QVBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH265QVbr.u32TargetBitRate = 1024 * 5 + 3072*u32FrameRate/30;	// ~8M bitrate
                        break;
                    case PIC_2592x1944:
            ...
            else if(SAMPLE_RC_CVBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
      		    case PIC_2304x1296:
                        stH265CVbr.u32MaxBitRate         = 1024 * 6 + 3072*u32FrameRate/30;
                        stH265CVbr.u32LongTermMaxBitrate = 1024 * 5 + 3072*u32FrameRate/30;
            ...
            if (SAMPLE_RC_CBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    case PIC_720P:
                        ...
                        break;
		    case PIC_360P:
                        stH264Cbr.u32BitRate = 256  * 1  + 256*u32FrameRate/30;
                        break;
                    case PIC_1080P:
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH264Vbr.u32MaxBitRate = 1024 * 5   + 2048*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
            ...
            else if (SAMPLE_RC_AVBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH264AVbr.u32MaxBitRate = 1024 * 5   + 3072*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
            ...
            else if (SAMPLE_RC_QVBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH264QVbr.u32TargetBitRate = 1024 * 5   + 3072*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
            ...
            else if(SAMPLE_RC_CVBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stH264CVbr.u32MaxBitRate         = 1024 * 6 + 3072*u32FrameRate/30;
                        stH264CVbr.u32LongTermMaxBitrate = 1024 * 5 + 3072*u32FrameRate/30;
            ...
            else if (SAMPLE_RC_CBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stMjpegeCbr.u32BitRate = 1024 * 15 + 3072*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
            ...
            else if (SAMPLE_RC_VBR == enRcMode)
            {
                ...
                switch (enSize)
                {
                    ...
                    case PIC_1080P:
                        ...
                        break;
		    case PIC_2304x1296:
                        stMjpegVbr.u32MaxBitRate = 1024 * 15 + 3072*u32FrameRate/30;
                        break;
                    case PIC_2592x1944:
...
HI_VOID* SAMPLE_COMM_YJSNPIVENC_GetVencStreamProc(HI_VOID* p)
{
    ...
    ******************************************/
	...
	pFile = fopen(pconf->save, "wb");
	if (!pFile) {
		SAMPLE_PRT("open file[%s] failed!\n", pconf->save);
		return NULL;
	}
	/* Set ch0 Venc Fd. */
	VencFd[0] = HI_MPI_VENC_GetFd(VencChn);
	if (VencFd[0] < 0) {
		SAMPLE_PRT("HI_MPI_VENC_GetFd failed with %#x!\n", VencFd[0]);
		return NULL;
	}
	...
	// ch1 send to udp
	VencChn = pstPara->VeChn[1];
	bzero(&s_addr, sizeof(s_addr));
	bzero(&d_addr, sizeof(d_addr));
	d_addr.sin_family = AF_INET;
	d_addr.sin_port = htons(pconf->dport);
	d_addr.sin_addr.s_addr = inet_addr(pconf->daddr);
	s_addr.sin_family = AF_INET;
	s_addr.sin_port = htons(pconf->lport);
	s_addr.sin_addr.s_addr = htonl(INADDR_ANY);
	if ((udp_sock = socket(PF_INET, SOCK_DGRAM, 0)) == -1) 
		printf("ERROR: Could not create UDP socket!");
	if (-1 == bind(udp_sock, (struct sockaddr*)&s_addr, sizeof(s_addr))) {
		fprintf(stderr, "Bind UDP port failed. \n");
		return (void *)HI_FAILURE;
	}
	
	/* Set ch1 Venc Fd. */
	VencFd[1] = HI_MPI_VENC_GetFd(VencChn);
	if (VencFd[1] < 0) {
		SAMPLE_PRT("HI_MPI_VENC_GetFd failed with %#x!\n", VencFd[i]);
		return NULL;
	}
	...
	s32Ret = HI_MPI_VENC_GetStreamBufInfo(VencChn, &stStreamBufInfo[1]);
	if (HI_SUCCESS != s32Ret) {
		SAMPLE_PRT("HI_MPI_VENC_GetStreamBufInfo failed with %#x!\n", s32Ret);
		return (void *)HI_FAILURE;
	}
            ...
            for (i = 0; i < s32ChnTotal; i++) {
                if (FD_ISSET(VencFd[i], &read_fds)) {
                    ...
                    *******************************************************/
					// 先写死 以后再说
					if (i == 0) {
						s32Ret = SAMPLE_COMM_YJSNPIVENC_SaveStreamToFile(pFile, &stStream);
						//printf("f");
					}
					else if (i == 1) {
						s32Ret = SAMPLE_COMM_YJSNPIVENC_SaveStreamToUdp(udp_sock, 
													(struct sockaddr*)&d_addr, &stStream);
						//printf("u");							
					}
					//printf("%d", i);
...
/******************************************************************************
* funciton : YJSNPI start get venc stream process thread
******************************************************************************/
HI_S32 SAMPLE_COMM_YJSNPIVENC_StartGetStream(VENC_CHN VeChn[], HI_S32 s32Cnt, YJSNPI_VENC_CONFIG_S *pconf)
{
    ...
    gs_stPara.bThreadStart = HI_TRUE;
    gs_stPara.s32Cnt = s32Cnt;
	memcpy(&gs_stPara.conf, pconf, sizeof(YJSNPI_VENC_CONFIG_S));
    for (i=0; i<s32Cnt; i++) {
        gs_stPara.VeChn[i] = VeChn[i];
    }
    return pthread_create(&gs_VencPid, 0, SAMPLE_COMM_YJSNPIVENC_GetVencStreamProc, (HI_VOID*)&gs_stPara);
}
