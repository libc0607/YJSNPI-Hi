...
#define IMX307_VMAX_720P60_LINEAR              (750+IMX307_INCREASE_LINES)
    ...
    switch (pstSnsState->u8ImgMode) {
        ...
        case IMX307_SENSOR_720P_60FPS_LINEAR_MODE:
            if ((f32Fps <= 60) && (f32Fps >= 0.12)) {
                u32VMAX = IMX307_VMAX_720P60_LINEAR * 60 / DIV_0_TO_1_FLOAT(f32Fps);
            } else {
                ISP_TRACE(HI_DBG_ERR, "Not support Fps: %f\n", f32Fps);
                return;
            }
...
static HI_VOID cmos_set_pixel_detect(VI_PIPE ViPipe, HI_BOOL bEnable)
{
    ...
    if (pstSnsState->enWDRMode == WDR_MODE_2To1_LINE) {
        ...
        } else if (pstSnsState->u8ImgMode == IMX307_SENSOR_720P_60FPS_LINEAR_MODE) {
            u32FullLines_5Fps = (IMX307_VMAX_720P60_LINEAR * 60) / 5;
        } else {
            return;
        }
    }
...
static HI_S32 cmos_set_wdr_mode(VI_PIPE ViPipe, HI_U8 u8Mode)
{
    ...
    switch (u8Mode & 0x3F) {
        ...
        case WDR_MODE_2To1_LINE:
            ...
            printf("2to1 line WDR mode(60fps->30fps)\n");
            break;
...
static HI_S32 cmos_set_image_mode(VI_PIPE ViPipe, ISP_CMOS_SENSOR_IMAGE_MODE_S *pstSensorImageMode)
{
    ...
    if (pstSensorImageMode->f32Fps <= 60) {
        if (pstSnsState->enWDRMode == WDR_MODE_NONE) {
            ...
            } else if (IMX307_RES_IS_720P(pstSensorImageMode->u16Width, pstSensorImageMode->u16Height)) {
