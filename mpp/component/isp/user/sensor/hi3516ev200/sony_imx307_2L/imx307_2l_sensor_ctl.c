 ...
 ******************************************************************************
  File Name     : imx307_2l_sensor_ctl.c
...
int imx307_2l_write_register(VI_PIPE ViPipe, HI_U32 addr, HI_U32 data)
{
    if (g_fd[ViPipe] < 0) {
        return HI_SUCCESS;
    }
    ...
    ret = write(g_fd[ViPipe], buf, imx307_2l_addr_byte + imx307_2l_data_byte);
    if (ret < 0) {
...
#define IMX307_SENSOR_720P_60FPS_LINEAR_MODE   (3)
...
void imx307_2l_default_reg_init(VI_PIPE ViPipe)
{
    ...
    for (i = 0; i < g_pastImx307_2l[ViPipe]->astRegsInfo[0].u32RegNum; i++) {
        imx307_2l_write_register(ViPipe, g_pastImx307_2l[ViPipe]->astRegsInfo[0].astI2cData[i].u32RegAddr, g_pastImx307_2l[ViPipe]->astRegsInfo[0].astI2cData[i].u32Data);
    }
}
...
void imx307_2l_init(VI_PIPE ViPipe)
{
    ...
    if (WDR_MODE_2To1_LINE == enWDRMode) {
		...
		if (IMX307_SENSOR_1080P_30FPS_LINEAR_MODE == u8ImgMode) {
    ...
    g_pastImx307_2l[ViPipe]->bInit = HI_TRUE;
    return;
}
...
void imx307_2l_linear_720p60_init(VI_PIPE ViPipe)
{
    ...
    imx307_2l_write_register(ViPipe, 0x3005, 0x01); // ADBIT 0-10 1-12
    imx307_2l_write_register(ViPipe, 0x3007, 0x10); // VREVERSE & HREVERSE & WINMODE(0x1)
    imx307_2l_write_register(ViPipe, 0x3009, 0x01); // FRSEL60p & FDG_SELlcg 
    ...
    imx307_2l_write_register(ViPipe, 0x301C, 0xE4); // HMAX;
    imx307_2l_write_register(ViPipe, 0x301D, 0x0C); // HMAX;
    imx307_2l_write_register(ViPipe, 0x3046, 0x01); // ODBIT & OPORTSEL
    imx307_2l_write_register(ViPipe, 0x304B, 0x0A); // XVSOUTSEL & XHSOUTSEL
    imx307_2l_write_register(ViPipe, 0x305C, 0x20); // INCKSEL1 37.125MHz
    ...
    imx307_2l_write_register(ViPipe, 0x3129, 0x00); // ADBIT1 1d/00
    ...
    imx307_2l_write_register(ViPipe, 0x317C, 0x00); // ADBIT2 12/00
    imx307_2l_write_register(ViPipe, 0x31EC, 0x0E); // ADBIT3 37/0e
    ...
    imx307_2l_write_register(ViPipe, 0x3418, 0xD9); // Y_OUT_SIZE
    imx307_2l_write_register(ViPipe, 0x3419, 0x02); // Y_OUT_SIZE
    imx307_2l_write_register(ViPipe, 0x3441, 0x0C); // CSI_DT_FMT 0a0a/0c0c
...
void imx307_2l_wdr_1080p30_2to1_init(VI_PIPE ViPipe)
{
    ...
    imx307_2l_write_register(ViPipe, 0x311C, 0x0E);
    imx307_2l_write_register(ViPipe, 0x3128, 0x04);
    imx307_2l_write_register(ViPipe, 0x3129, 0x1D); //  ADBIT1,12Bit; 0x1D->10Bit;
    imx307_2l_write_register(ViPipe, 0x313B, 0x41);
    ...
    imx307_2l_write_register(ViPipe, 0x317C, 0x12); //  ADBIT2,12Bit;0x12->10Bit;
    imx307_2l_write_register(ViPipe, 0x31EC, 0x37); //  ADBIT3,12Bit;0x37->10Bit;

    // MIPI setting
    imx307_2l_write_register(ViPipe, 0x3405, 0x00); //  REPETITION
    imx307_2l_write_register(ViPipe, 0x3407, 0x01); //  VREVERSE & HREVERSE & WINMODE
    imx307_2l_write_register(ViPipe, 0x3414, 0x0A); //  OPB_SIZE_V
